...
var database = {
    clients: {
        krunal: {secret: "krunal"},
        unofficialClient: {secret: "DECAF"}
    },
    users: {
        krunal: {password: "krunal"}
    },
    tokensToUsernames: {
        krunal: "ASH"
    }
...
function generateToken(data) {
    ...
    var sha256 = G.crypto.createHmac("sha256", random + "WOO" + timestamp);

    return sha256.update(data).digest("base64");
}
module.exports = {
    validateClient: function(credentials, req, cb) {
    ...
    grantUserToken: function(credentials, req, cb) {
